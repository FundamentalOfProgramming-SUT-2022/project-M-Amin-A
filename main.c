...
//word by word compratore

//undo save files and tree hide files
...
int input()
{
    ...
    else if(strcmp(cmd,"insertstr")==0)
    {
        ...
        scanf("%d:%d",&line_index,&char_index);
    ...
    else if(strcmp(cmd,"find")==0 || strcmp(cmd,"replace")==0)
    {
        ...
        if(strcmp(cmd,"find")==0)
        {
            if(is_arman==0)
            {
                find(path,str,count,at,byword,all,stdout,0,"");
            }
            else
            {
                FILE* ar_file=fopen("system/arman_output.txt","w");
        ...
        else
        {
            ...
            int res=find(path,str,count,at,byword,all,stdout,1,str2);
            if(res==0)
	        {
	        	save_file(path);
			}
    ...
    else if(strcmp(cmd,"grep")==0)
    {
        ...
        if(is_arman==0)
        {
            grep(str,files_count,files,flag,stdout);
        }
        else
        {
            FILE* ar_file=fopen("system/arman_output.txt","w");
    ...
    else if(strcmp(cmd,"tree")==0)
    {
        ...
        {
            printf("root\n");
            tree(depth,0,c_path,c_pre,stdout);
        }
        else
        {
            FILE* ar_file=fopen("system/arman_output.txt","w");
...
int arman()
{
    ...
    int cmd_out=process_input(cmd,0);
    if(cmd_out==-1)
    {
        syntax_error("unexpected backslash.",-1);
    }
